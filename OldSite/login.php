...
if ($action == "login" 
	...
	echo "opener.location = \"events.php?month=$m&year=$y\";";
	echo "window.setTimeout('window.close()', 500);";
	echo "</script>";
} elseif ($action == "logout") {
	session_start();
	session_destroy();
	header ("Location: events.php?month=$m&year=$y");
} else {
?>
	<html>
	<head>
...
?>
	...
	<table>
	<form action="<?php echo $_SERVER['PHP_SELF'] ?>?action=login&month=<?php echo $m ?>&year=<?php echo $y ?>" method="post">
			<tr>
